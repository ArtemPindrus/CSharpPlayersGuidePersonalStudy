...
namespace TheUncodedOneBattle.Players {
    class User : IPlayer {
        public ICharacterAction PickAction(Character performant) {
            ...
            CharacterActionEnum action = (CharacterActionEnum) ConsoleExtensions.GetInt($"User, choose the action for {performant.Name}: ", 1, CharacterActionHelper.ActionAmount)-1;
            ...
            if (action == CharacterActionEnum.DoNothing) return new DoNothing(performant);
                ...
                int target = oppositeParty.Count == 1 ? 0 : ConsoleExtensions.GetInt("User, choose the target: ", 1, oppositeParty.Count) - 1;
                ...
                int target = party.Items.Count == 1 ? 0 : ConsoleExtensions.GetInt("User, choose the item: ", 1, party.Items.Count) - 1;
